...
# --- 3. 关键事件定义 ---
event_list = [
    ("25.11.23: 3#一开钻进", "2025-11-23"),
    ("25.11.26: 移至4#", "2025-11-26"),
    ("25.12.19: 4#移至5#", "2025-12-19"),
    ("26.01.09: 5#移至3#", "2026-01-09"),
    ("26.01.23: 3#移至7#", "2026-01-23"),
    ("26.01.23: 7#移至6#", "2026-01-23"),
    ("26.02.13: 7#遇起钻最高摩阻38t", "2026-02-13"),
    ("26.03.02: 6#二开钻进", "2026-03-02"),
    ("26.03.12: 6#二开钻进", "2026-03-12"),
    ("26.04.13: 4#三开钻进", "2026-04-13"),
...
with col2:
    selected_nodes = st.multiselect("选择监测节点:", options=st.session_state.df.index.tolist(), default=st.session_state.df.index.tolist()[:3])
...
if selected_nodes:
    ...
    for i, node in enumerate(selected_nodes):
        fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df[node], mode='lines+markers', name=str(node),
                                 marker=dict(symbol=symbols[i % len(symbols)], size=8)))

    # 添加关键日期竖线
    for _, date_str in event_list:
        fig.add_vline(x=pd.to_datetime(date_str), line_dash="dash", line_color="gray", opacity=0.5)
    ...
    fig.update_layout(xaxis=xaxis_config, yaxis=dict(title="压力值 (kPa)"), plot_bgcolor='white', height=500, hovermode="x unified")
